...
drugs_day2 <- function(n_pat,n_day,drugtaget,n_drug_day,time){
  ...
  #calculate whether have DDI during the specific timeframe;
  #overlap of the drugs
  n_drug_day[n_drug_day < 1] <-0
  n_drug_day[n_drug_day > 0] <-1
  #base drug + potential ddi frame;
  ddi_have<-n_drug_day + drug
  #for have at least one in one day;
  ddi_have[ddi_have < 2] <-0
  ddi_have[ddi_have > 1] <-1
  ddi_have_time <-ddi_have[, 1:time]
  #proportion of DDI within the assessment period;
  ddi_have_timea<-as.data.frame(t(ddi_have_time ))
  colsum <- function(data) apply(data, 2, sum)
  ddi_proportion<-colsum(ddi_have_timea)
  ddi_proportion<-as.matrix(ddi_proportion)
  proportion<-mean(ddi_proportion)/time
  proportion
